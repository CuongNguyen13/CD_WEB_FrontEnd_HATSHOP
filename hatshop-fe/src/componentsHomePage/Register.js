...
function Register() {
  ...
  useEffect(() => {
    console.log("hello")
    registerApi.checkEmailExist(checkEmail).then(res => {
  ...
  function checkEmailHandle(email1) {
    setCheckEmail(email1);
    console.log(checkEmail)
    const newInput = { ...formik.values, email:email1 }
  ...
  return (
    ...
    <div className="wrapper" style={{ margin: "150px auto", width: "80%" }}>
      ...
      <form className="form-right" style={{ backgroundColor: "" }} onSubmit={formik.handleSubmit}>
        ...
        <div className="mb-3">
          <input type="email" className="input-field" name="email" id="email" required placeholder='Email' value={formik.values.email} onChange={(event) => checkEmailHandle(event.target.value)} />
